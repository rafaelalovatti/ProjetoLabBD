    ...
    try:
        with db.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            if tipo == "Escuderia":
                ...
                print(f"Resultado da consulta de nome: {escuderia_nome}")
                extra_info = f"Escuderia: {escuderia_nome['name'] if escuderia_nome else 'ID ' + str(escuderia_id)}"
                
                # Quantidade de vitórias
                cur.execute(
                    """
                    SELECT Public.ReportConstructorWins(%s) AS wins
                    """,
                    (escuderia_id,),
                )
                wins = cur.fetchone()
                ...
                cur.execute(
                    """
                    SELECT Public.ReportDistinctDrivers(%s) AS driver_count
                    """,
                    (escuderia_id,),
                )
                driver_count = cur.fetchone()
                print(f"Pilotos diferentes: {driver_count}")
                driver_count = driver_count['driver_count'] if driver_count else 0
                
                # Primeiro e último ano
                cur.execute(
                    """
                    SELECT FirstYear, LastYear
                    FROM Public.ReportConstructorYears(%s)
                    """,
                    (escuderia_id,),
                )
                years = cur.fetchone()
                ...
                print(f"Resumo Escuderia: {resumo}")
            
            elif tipo == "Administrador":
                extra_info = "Administrador - acesso total"
                cur.execute("SELECT COUNT(*) AS total_users FROM Users")
                resumo = {"Total de usuários": cur.fetchone()['total_users']}
